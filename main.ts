import {Vector, bool} from './classes/DataStructures'
import { ExampleData } from './classes/ExampleData';
...
canvas.addEventListener('mousedown', function(e) {
  ...
  if(state.guessing) {
    ...
    (<HTMLInputElement>document.getElementById('guess')).style.backgroundColor = state.colors[Math.round(state.guess)];
...
document.getElementById('btn-change').onclick = () => {
  ...
  (<HTMLElement>document.querySelector(':root')).style.setProperty('--team-color', state.colors[state.currentTeam]);
};
